...
export const PlayingCard: React.FC<PlayingCardProps> = ({
  ...
  return (
    <div
      className={`
        relative ${sizeClass} perspective-1000 cursor-pointer select-none
        transition-all duration-300 cubic-bezier(0.25, 0.8, 0.25, 1)
        ${isSelected ? (isMobileLandscape ? '-translate-y-2 z-10' : '-translate-y-4 md:-translate-y-6 z-10') : 'hover:-translate-y-2 md:hover:-translate-y-3 hover:scale-105 hover:z-10'}
    ...
    >
      ...
      >
        {/* Card Back */}
        <div className={`absolute inset-0 w-full h-full backface-hidden overflow-hidden bg-casino-green-800 border-2 border-gold-600 shadow-inner ${isMobileLandscape ? 'rounded-sm' : 'rounded-md sm:rounded-lg md:rounded-xl'}`}>
          ...
          <div className={`absolute border border-gold-500/50 ${isMobileLandscape ? 'inset-0.5 rounded-sm' : 'inset-1 md:inset-2 rounded-sm md:rounded-lg'}`}></div>
          <div className="absolute inset-0 flex items-center justify-center">
             <div className={`rounded-full border-2 border-gold-400 flex items-center justify-center bg-casino-green-900 shadow-lg ${isMobileLandscape ? 'w-5 h-5' : 'w-8 h-8 sm:w-10 sm:h-10 md:w-14 md:h-14 lg:w-16 lg:h-16'}`}>
                <span className={`font-playfair text-gold-300 ${isMobileLandscape ? 'text-[8px]' : 'text-sm sm:text-lg md:text-xl lg:text-2xl'}`}>CP</span>
             </div>
          </div>
        </div>
        ...
        `}>
           ...
           <div className={`absolute ${isMobileLandscape ? 'top-0.5 left-0.5' : 'top-0.5 left-0.5 sm:top-1 sm:left-1 md:top-2 md:left-2'} flex flex-col items-center ${colorClass}`}>
             <span className={`font-playfair font-bold leading-none tracking-tighter ${isMobileLandscape ? 'text-[10px]' : 'text-base sm:text-lg md:text-xl lg:text-2xl'}`}>{rank}</span>
             <SuitIcon suit={suit} className={`${isMobileLandscape ? 'w-2 h-2 mt-0' : 'w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-4 md:h-4 lg:w-5 lg:h-5 mt-0.5 md:mt-1'}`} />
           </div>

           {/* Center Suit */}
           <div className={`absolute inset-0 flex items-center justify-center ${colorClass}`}>
             <SuitIcon suit={suit} className={`opacity-100 drop-shadow-sm ${isMobileLandscape ? 'w-5 h-5' : 'w-8 h-8 sm:w-10 sm:h-10 md:w-16 md:h-16 lg:w-20 lg:h-20'}`} />
           </div>

           {/* Corner Bottom-Right (Rotated) */}
           <div className={`absolute ${isMobileLandscape ? 'bottom-0.5 right-0.5' : 'bottom-0.5 right-0.5 sm:bottom-1 sm:right-1 md:bottom-2 md:right-2'} flex flex-col items-center ${colorClass} rotate-180`}>
             <span className={`font-playfair font-bold leading-none tracking-tighter ${isMobileLandscape ? 'text-[10px]' : 'text-base sm:text-lg md:text-xl lg:text-2xl'}`}>{rank}</span>
             <SuitIcon suit={suit} className={`${isMobileLandscape ? 'w-2 h-2 mt-0' : 'w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-4 md:h-4 lg:w-5 lg:h-5 mt-0.5 md:mt-1'}`} />
           </div>
